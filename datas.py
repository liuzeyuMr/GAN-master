...
class mnist():
	...
	def __call__(self,batch_size):
		batch_imgs,y = self.data.train.next_batch(batch_size) #batch_imgs.shape==(64,784)  y.shape==（64，10）
		if self.flag == 'conv':#这里判断是不是卷积操作
			batch_imgs = np.reshape(batch_imgs, (batch_size, self.size, self.size, self.channel)) 
		if self.is_tanh:
			batch_imgs = batch_imgs*2 - 1#使用thanh激活函数，使像素值分布在-1到1之间
