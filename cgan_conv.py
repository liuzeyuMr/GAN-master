...
def conv_concat(x,y):
	...
	# y * tf.ones([bz, 28, 28, 10]) 可以想象成是有通道数为10的图片，然后只有为1的那个通道值为1。其余的全为0。作为条件
...
class CGAN():
	def __init__(self, generator, discriminator, data):
		...
		self.size = self.data.size#28
		self.channel = self.data.channel#1
		...
		# nets
		self.G_sample = self.generator(concat(self.z, self.y))#输入还是（64，110）带条件噪音输入

		self.D_real, _ = self.discriminator(conv_concat(self.X, self.y))#这里输入的是真实图片加标签
	...
	def train(self, sample_dir, ckpt_dir='ckpt', training_epoches = 1000000, batch_size = 64):
		...
		for epoch in range(training_epoches):
			...
			if epoch % 100 == 0 or epoch < 100:
				...
				if epoch % 1000 == 0:#每一千次保存一次sample出来的图像
...
if __name__ == '__main__':
	...
	sample_dir = 'D:\Samples\mnist_cgan_conv'
